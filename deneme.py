...
def create_filters(window_size, out_size, low=50, high=6000):
    x = np.arange(0, window_size, 1)
    wsin = np.empty((out_size, 1, window_size), dtype=np.float32)
    wcos = np.empty((out_size, 1, window_size), dtype=np.float32)
    start_freq = low
    end_freq = high
    ...
    for ind in range(out_size):
        wsin[ind,0,:] = window_mask*np.sin(2 * np.pi * ind / out_size * x)
        wcos[ind,0,:] = window_mask*np.cos(2 * np.pi * ind / out_size * x)
